...
const { generate, generateSong, generatePopSong, generateHook } = createRequire(import.meta.url)(join(tmp, 'engine.cjs'));
...
// ---------- 팝 구성 완성곡 ----------
test('generatePopSong: 팝 섹션 구성 + 섹션 주석 + 종지선', () => {
  const out = generatePopSong({ key: 'major', seed: 5 });
  assert.ok(out.totalBars >= 40, '전체 곡 길이(약 60마디): ' + out.totalBars);
  const labels = out.sections.map(s => s.label);
  ['인트로', '벌스', '프리코러스', '코러스', '간주', '브릿지', '아웃트로'].forEach(l =>
    assert.ok(labels.includes(l), '섹션 포함: ' + l));
  ['── 인트로 ──', '── 코러스 ──', '── 아웃트로 ──'].forEach(mk =>
    assert.ok(out.abc.includes(mk), '섹션 주석 ' + mk));
  assert.ok(out.abc.trim().endsWith('|]'), '완성곡은 |]로 종료');
});

test('generatePopSong: 반복 코러스는 동일(후렴 일관성), 시드 재현성', () => {
  const out = generatePopSong({ key: 'major', seed: 11 });
  const chor = out.sections.filter(s => s.label === '코러스');
  assert.ok(chor.length >= 2, '코러스 2개 이상');
  const key = s => s.bars.map(b => b.notes.join('.') + '/' + b.rhythm.join('.')).join('|');
  assert.equal(key(chor[0]), key(chor[1]), '반복 코러스는 같은 후렴');
  const a = generatePopSong({ key: 'major', seed: 11 });
  assert.equal(a.abc, out.abc, '같은 시드 → 동일 곡');
});

test('generatePopSong: 강박=코드톤, 아웃트로 으뜸화음 착지, 단조=Am 조표', () => {
  const out = generatePopSong({ key: 'major', seed: 3 });
  // 각 섹션 첫 마디 첫 음은 그 코드 구성음
  out.sections.forEach(sec => {
    const b0 = sec.bars[0];
    assert.ok(chordPcs(b0.chord).includes(b0.notes[0] % 12), sec.label + ' 첫 음=코드톤');
  });
  const minor = generatePopSong({ key: 'minor', seed: 3 });
  assert.ok(/\nK:Am\n/.test(minor.abc), '단조는 K:Am 조표');
});

// ---------- 훅(후렴) 엔진 ----------
test('generateHook: 도입·훅·마무리 구성 + 훅 길이 + 종지', () => {
  const out = generateHook({ key: 'major', hookBars: 8, seed: 5 });
  assert.equal(out.sections.length, 3, '도입·훅·마무리');
  assert.deepEqual(out.sections.map(s => s.label), ['도입(무난)', '훅(후렴)', '마무리(무난)']);
  const hook = out.sections[1].bars;
  assert.equal(hook.length, 8, '훅 8마디');
  ['── 도입(무난) ──', '── 훅(후렴) ──', '── 마무리(무난) ──'].forEach(mk =>
    assert.ok(out.abc.includes(mk), '섹션 주석 ' + mk));
  assert.ok(out.abc.trim().endsWith('|]'), '|]로 종료');
  // 각 마디: 음 개수 = 리듬 슬롯 수, 마디합 8
  hook.forEach(b => {
    assert.equal(b.notes.length, b.rhythm.length);
    assert.equal(b.rhythm.reduce((a, d) => a + d, 0), 8, '훅 마디합 8');
  });
});

test('generateHook: 훅은 2마디 모티프를 반복(리듬·정점 반복)한다', () => {
  const out = generateHook({ key: 'major', hookBars: 8, seed: 7 });
  const hb = out.sections[1].bars;
  // 블록(2마디) 리듬이 반복되어야 각인된다 — 홀수 마디끼리, 짝수 마디끼리 같은 리듬
  assert.deepEqual(hb[0].rhythm, hb[2].rhythm, '블록1·2 첫 마디 리듬 동일(반복)');
  assert.deepEqual(hb[1].rhythm, hb[3].rhythm, '블록1·2 둘째 마디 리듬 동일(반복)');
  // 정점음(가장 긴 음)이 존재 — 각인점
  const hasPeak = hb.some(b => b.rhythm.includes(4));
  assert.ok(hasPeak, '정점음(긴 음) 존재');
});

test('generateHook: 강박=코드톤, 훅 끝 으뜸음, 시드 재현성', () => {
  const out = generateHook({ key: 'major', hookBars: 4, seed: 3 });
  const hb = out.sections[1].bars;
  hb.forEach(b => {
    const pcs = chordPcs(b.chord);
    let onset = 0;
    for (let k = 0; k < b.notes.length; k++) {
      if (onset % 4 === 0) assert.ok(pcs.includes(b.notes[k] % 12), '훅 강박 코드톤: ' + b.chord + ' ' + b.notes[k]);
      onset += b.rhythm[k];
    }
  });
  const lastHookBar = hb[hb.length - 1];
  assert.equal(lastHookBar.notes[lastHookBar.notes.length - 1] % 12, 0, '훅 끝 으뜸음(C)');
