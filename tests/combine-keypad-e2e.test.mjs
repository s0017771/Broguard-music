...

{
  ...
  try {
    ...
    for (const k of ['a', 's', 'd']) await page.keyboard.press(k);  // 도 레 미 (a=보통 도, 중앙)
    await page.keyboard.press('Space');                            // 도'(높은 도)
    ...
    await page.keyboard.press('u');                                // 솔'(높은 솔)
    await page.keyboard.press('p');                                // 맨 위 도(도'')
    ...
    ok('알파벳=멜로디(옥타브 반영) · 숫자=리듬 · 스페이스=높은도 · Q=마디 · T=쉼표');

    // 4) u = 솔'(높은 솔)
    await clear(); await page.focus('#melodyIn'); await page.keyboard.press('u');
    assert.equal(await page.inputValue('#melodyIn'), "솔' ", "u = 높은 솔(솔')");
    ok('u 키가 높은 솔로 입력된다');
    ...
    assert.equal(await page.inputValue('#melodyIn'), '도 ', '리듬 칸에서 친 알파벳 → 멜로디(보통 도)');
