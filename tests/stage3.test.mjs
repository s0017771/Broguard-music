...
test('buildVocalBrief: 스타일 태그 + 섹션 태그 가사를 만든다(송 생성 AI용)', () => {
  const brief = Stage3.buildVocalBrief(PLAN, BLOCKS);
  assert.ok(brief.includes('[Style]') && brief.includes('[Lyrics]'));
  assert.ok(brief.includes('tempo: ' + PLAN.tempo + ' BPM'));
  assert.ok(brief.includes('genre: ' + PLAN.genre) && brief.includes('Korean vocal'));
  // 섹션 이름은 영어 태그로
  assert.ok(brief.includes('[Verse]') && brief.includes('[Chorus]'));
  assert.ok(brief.includes('거울 속 흰머리'), '가사 본문 포함');
});
